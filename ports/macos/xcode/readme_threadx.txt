                         Microsoft's Azure RTOS ThreadX for macos
...
macos-specific initialization is done in the function _tx_initialize_low_level,
...
address for use by the application. In macos, this is basically done
by using malloc to get a big block of memory from macos.


4.  macos Implementation

ThreadX for macos is implemented using POSIX pthreads. Each application
thread in ThreadX actually runs as a macos pthread. The determination of
which application thread to run is made by the ThreadX scheduler, which
itself is a macos pthread. The ThreadX scheduler is the highest priority
thread in the system.

Interrupts in ThreadX/macos are also simulated by pthreads. A good example
is the ThreadX system timer interrupt, which can be found in
tx_initialize_low_level.c.

ThreadX for macos utilizes the API pthread_setschedparam() which requires
...
ThreadX provides simulated interrupt handling with macos pthreads. Simulated
...
Here is an example of how to define the macos data structures and prototypes
necessary to create a simulated interrupt thread:

pthread_t    _sample_macos_interrupt_thread;
void        *_sample_macos_interrupt_entry(void *p);

6.2  Creating a Simulated Interrupt Thread

Here is an example of how to create a simulated interrupt thread in macos.
...
struct sched_param sp;
       ...
       pthread_create(&_sample_macos_interrupt_thread, NULL, _sample_macos_interrupt_entry, &_sample_macos_interrupt_thread);

       /* Set up the ISR priority */
       sp.sched_priority = TX_macos_PRIORITY_ISR;
       pthread_setschedparam(_sample_macos_interrupt_thread, SCHED_FIFO, &sp);
...
09-30-2020  Initial ThreadX 6.1 version for macos using GNU GCC tools.
